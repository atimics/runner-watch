(() => {
  ...
  const labels = {pending: 'Queued', sending: 'Sending', sent: 'Delivered', retry: 'Waiting to retry', uncertain: 'Check delivery', failed: 'Needs attention', stale: 'Past its news window'};
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      for (const post of data.posts) {
        ...
        card.append(header, element('p', `${post.format === 'animation' ? 'Coin replay GIF' : 'Text update'} · Destination ${post.chat_id}`), element('pre', post.display_text || post.text || (post.status === 'sent' ? 'Earlier delivery · Open the saved replay for its evidence.' : 'The caption will appear when the replay is ready.')));
