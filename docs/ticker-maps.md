...
People use SEC CIKs where available. Name-based matches carry a visible label. Small blue, red and gray circles around each person link straight to other stocks. Cross-stock matches use SEC CIKs. Interests use the latest saved filing line per relationship and share class. Blue circles show buys or stakes, red circles show sales, and gray circles show roles or other activity. Stake percentage and trade value use separate size scales. The existing map loads older connection pages in the background. The centre holds the score; contributions and penalties share one row in the source panel. Filing notes appear as plain text. Older aggregate rows carry “Filing summary.” Each collector cycle attempts to restore 20 older rows from saved SEC XML documents. The restore cursor lives in `worker_state.stock_map_restore_after`; deleting that key starts another archive pass.
