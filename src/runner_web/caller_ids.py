    ...
    with connection() as database:
        ...
        if clean_reference:
            existing = database.execute(
                """
                SELECT i.id,i.handle,i.status,i.claim_cost_cents,i.claimed_at,c.user_id
                FROM caller_identity_claims c
                JOIN caller_identities i ON i.id=c.caller_identity_id
        ...
        claimed_at = _iso()
        for _ in range(len(ADJECTIVES) * len(ANIMALS)):
            identity_id = str(uuid.uuid4())
            handle = f"{secrets.choice(ADJECTIVES)}-{secrets.choice(ANIMALS)}"
