...
def _golf_round_cards(competitor: dict[str, Any]) -> list[dict[str, Any]]:
    cards = []
    for raw in competitor.get("linescores") or []:
        try:
            number = int(raw.get("period"))
        except (TypeError, ValueError):
            continue
        if not 1 <= number <= 6:
            continue
        holes = len(raw.get("linescores") or [])
        strokes = _number(raw.get("value"))
        cards.append(
            {
                "number": number,
                "strokes": int(strokes) if strokes is not None and strokes.is_integer() else None,
                "to_par": str(raw.get("displayValue") or ""),
                "holes": holes,
                "complete": holes >= 18,
            }
        )
...
def normalize_golf_event(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not external_id:
        return None
    raw_competitors = competition.get("competitors") or []
    team_competitors = [
        competitor
        ...
        teams.append(
            {
                ...
                "abbreviation": str(team.get("abbreviation") or ""),
                "points": _number(competitor.get("score")),
                "points_display": str(competitor.get("score") or "0"),
            }
        )
        ...
        leaderboard.append(
            {
                ...
                "rounds": _golf_round_cards(competitor),
            }
        )
    leaderboard.sort(
        key=lambda item: (
    ...
    score_positions: dict[float, list[int]] = defaultdict(list)
    for player in leaderboard:
        if player["score"] is not None and player["position"] is not None:
            score_positions[player["score"]].append(player["position"])
    for player in leaderboard:
        positions = score_positions.get(player["score"])
        if positions and len(positions) > 1 and player["position_display"].isdigit():
            player["position_display"] = f"T{min(positions)}"
    ...
    end_time = _parse_time(competition.get("endDate") or event.get("endDate") or event.get("date"))
    return {
...
def predict_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    if home_record and away_record:
        ...
        quality = "thin"
    home_venue_delta = float(LEAGUES[event["league"]]["home_edge"])
    probability += home_venue_delta
    home_probability = max(0.18, min(0.82, probability))
    away_probability = 1 - home_probability
    factors = {
        "baseline_pct": 50.0,
        "home_record_delta_pp": round(home_record_delta * 100, 6),
...
def fetch_golf(at: datetime | None = None) -> list[dict[str, Any]]:
    ...
    date_range = (
        f"{current.date() - timedelta(days=10):%Y%m%d}-{current.date() + timedelta(days=14):%Y%m%d}"
    )
            ...
            event.update(
                {
                    ...
                    "odds_source_updated_at": str(row["source_observed_at"] or row["observed_at"]),
                }
            )
            applied += 1
    return applied
...
def store_golf_events(
    ...
    with connection() as database:
        for event in events:
            ...
            if leaderboard or event.get("scoring_format") == "match_play":
                database.execute(
            ...
            for player in leaderboard:
                database.execute(
                    ...
                    INSERT INTO sports_golf_leaderboard(
                        ...
                        score,score_display,through_display,round_number,round_display,
                        collected_at,rounds_json
                    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (
                        event["id"],
    ...
    with connection() as database:
        for event in events:
                        ...
                        _json(
                            {
                                ...
                                "previous_snapshot_id": (
                                    str(previous_market["id"]) if previous_market else None
                                ),
                            }
                        ).encode()
                    ).hexdigest()
            ...
            previous_input_state = (
                str(previous_prediction["input_state_hash"] or previous_prediction["input_hash"])
                if previous_prediction
                else None
            )
            needs_pregame_factors = bool(
                previous_prediction
            ...
            if (
                    ...
                    _json(
                        {
                            ...
                            "previous_prediction_id": (
                                str(previous_prediction["id"]) if previous_prediction else None
                            ),
                        }
                    ).encode()
                ).hexdigest()
                database.execute(
                    ...
                    INSERT INTO sports_predictions(
                        ...
                        edge,signal,quality,evidence_json,risks_json,factors_json,observed_at
                    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING
                    """,
                    (
                        str(uuid.uuid4()),
    ...
    with connection() as database:
        ...
        for row in rows:
            ...
            terminal_without_result = status == "post" and (
                not bool(row["completed"]) or home_score is None or away_score is None
            )
...
def _prediction_receipt(
    ...
    sealed = bool(
        event.get("completed") or event.get("status") != "pre" or start <= datetime.now(UTC)
    )
...
def _event_rows(database: Any, rows: list[Any]) -> list[dict[str, Any]]:
    ...
    prediction_rows = database.execute(
        ...
        WITH eligible_models AS (
            SELECT p.* FROM sports_predictions p
...
def golf_slate(limit: int = 6, leaderboard_limit: int = 10) -> dict[str, Any]:
    ...
    with connection() as database:
        ...
        for row in rows:
            ...
            for player in leaders:
                entry = dict(player)
                entry["rounds"] = json.loads(entry.pop("rounds_json") or "[]")
...
def golf_event(event_id: str) -> dict[str, Any] | None:
    with connection() as database:
        row = database.execute(
        ...
        for player in database.execute(
            "SELECT * FROM sports_golf_leaderboard WHERE event_id=? "
            "ORDER BY CASE WHEN position IS NULL THEN 1 ELSE 0 END, position,player_name",
            (event_id,),
        ).fetchall():
            entry = dict(player)
            entry["rounds"] = json.loads(entry.pop("rounds_json") or "[]")
...
def golf_market_context(event: dict[str, Any]) -> dict[str, Any]:
    ...
    round_numbers = sorted(
        {
            ...
            for card in player.get("rounds") or []
            if card.get("complete") and card.get("strokes") is not None
        }
    )
    rounds = []
    for number in round_numbers:
        cards = [
            ...
            for player in players
            for card in player.get("rounds") or []
            if card["number"] == number and card.get("complete") and card.get("strokes") is not None
        ]
        low_strokes = min(card["strokes"] for _, card in cards)
        low_names = [
            player["player_name"] for player, card in cards if card["strokes"] == low_strokes
        ]
        after_round = []
        for player in players:
            complete = {
                card["number"]: card["strokes"]
                for card in player.get("rounds") or []
                if card.get("complete") and card.get("strokes") is not None
            }
            if all(previous in complete for previous in range(1, number + 1)):
                total = sum(complete[previous] for previous in range(1, number + 1))
                after_round.append((player["player_name"], total))
        best_total = min((total for _, total in after_round), default=None)
        rounds.append(
            {
                "number": number,
                "low_strokes": low_strokes,
                "low_names": low_names,
                "leader_names": [name for name, total in after_round if total == best_total],
                "leader_strokes": best_total,
            }
        )

    leaders = []
    if event.get("status") != "pre":
        scores = [player["score"] for player in players if player.get("score") is not None]
        if scores:
            best = min(scores)
            leaders = [player for player in players if player.get("score") == best]
    return {"format": "stroke_play", "leaders": leaders, "rounds": rounds}
...
def _sports_series_key(event: dict[str, Any]) -> str:
    team_ids = sorted((str(event.get("away_team_id") or ""), str(event.get("home_team_id") or "")))
...
def _sample_assessment(games: int) -> dict[str, Any]:
    if games < 25:
        ...
        message = (
            "Too few settled forecasts to judge the model. These numbers are a receipt, not proof."
        )
    elif games < 100:
        label = "EARLY SAMPLE"
        ...
        message = (
            "The scorecard has reached the public review target. Calibration still matters most."
        )
    return {
        "label": label,
        "message": message,
...
def _build_model_alpha(league: str) -> dict[str, Any]:
    ...
    for row in current_rows:
        ...
        if row.get("edge_prediction_id") and edge_selection in {"home", "away"}:
            ...
            edge_odds = (
                row.get("edge_home_odds") if edge_selection == "home" else row.get("edge_away_odds")
            )
...
def _game_view_state(event: dict[str, Any], current: datetime | None = None) -> dict[str, Any]:
    ...
    home_score = _number(event.get("home_score"))
    away_score = _number(event.get("away_score"))
    score_available = status in {"in", "post"} and home_score is not None and away_score is not None
...
def _series_context(database: Any, event: dict[str, Any]) -> dict[str, Any]:
    ...
    back_to_back = bool(between_starts and timedelta(0) < between_starts <= BACK_TO_BACK_MAX_GAP)
    if previous:
...
def _matchup_player_context(database: Any, event: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    roster_event_by_team = {str(row["team_id"]): str(row["event_id"]) for row in roster_events}
...
def sports_event(event_id: str) -> dict[str, Any] | None:
    with connection() as database:
        ...
        event["odds_history"] = [
            odds_item for item in reversed(odds_rows) if (odds_item := _odds_item(item)) is not None
        ]
...
def sports_team_profile(provider: str, league: str, team_id: str) -> dict[str, Any] | None:
    if league not in LEAGUES:
        return None
    with connection() as database:
        rows = database.execute(
            """
            SELECT * FROM sports_events
            WHERE provider=? AND league=? AND (home_team_id=? OR away_team_id=?)
            ORDER BY start_time DESC,id DESC LIMIT 40
            """,
            (provider, league, team_id, team_id),
        ).fetchall()
        if not rows:
            return None
        latest = dict(rows[0])
        side = "home" if str(latest["home_team_id"]) == team_id else "away"
        appearances = database.execute(
            """
            SELECT a.player_id,a.player_name,a.position,e.start_time
            FROM sports_player_appearances a
            JOIN sports_events e ON e.id=a.event_id
            WHERE e.provider=? AND a.league=? AND a.team_id=?
            ORDER BY e.start_time DESC,a.player_name LIMIT 120
            """,
            (provider, league, team_id),
        ).fetchall()
    games = []
    for raw in rows:
        event = dict(raw)
        own_side = "home" if str(event["home_team_id"]) == team_id else "away"
        other_side = "away" if own_side == "home" else "home"
        ...
        games.append(
            {
                "id": event["id"],
                "href": "/game/" + quote(str(event["id"]), safe=":"),
                "opponent": event[f"{other_side}_team_name"],
                "side": "Home" if own_side == "home" else "Away",
                "score": f"{int(own_score)} – {int(other_score)}" if scored else "vs",
                "status": result,
        ...
        players.append(
            {
                ...
                "href": "/player/"
                + "/".join(quote(str(value), safe="") for value in (provider, league, player_id)),
            }
        )
        if len(players) == 24:
            break
    return {
        "kind": "team",
        "name": latest[f"{side}_team_name"],
        "abbreviation": latest[f"{side}_abbreviation"],
        "league": league.upper(),
        "record": latest.get(f"{side}_record") or "Record pending",
        "games": games,
        "players": players,
    }


def sports_player_profile(provider: str, league: str, player_id: str) -> dict[str, Any] | None:
    if league == "golf":
        with connection() as database:
            rows = database.execute(
                """
                SELECT g.player_name,g.country,g.score_display,g.position_display,
                       e.id AS event_id,e.name AS event_name,e.start_time
                FROM sports_golf_leaderboard g
                JOIN sports_golf_events e ON e.id=g.event_id
                WHERE e.provider=? AND g.player_id=?
                ORDER BY e.start_time DESC,e.id DESC LIMIT 40
                """,
                (provider, player_id),
            ).fetchall()
        if not rows:
            ...
            "games": [
                {
                    ...
                    "opponent": row["event_name"],
                    "score": row["score_display"],
                    "status": row["position_display"],
    ...
    with connection() as database:
        rows = database.execute(
            """
            SELECT a.player_name,a.position,a.team_id,a.team_name,a.team_abbreviation,
                   a.stats_json,e.id AS event_id,e.start_time,e.provider,
                   e.home_team_id,e.home_team_name,e.away_team_name
            FROM sports_player_appearances a
            JOIN sports_events e ON e.id=a.event_id
            WHERE e.provider=? AND a.league=? AND a.player_id=?
            ORDER BY e.start_time DESC,e.id DESC LIMIT 40
            """,
            (provider, league, player_id),
        ).fetchall()
    if not rows:
        return None
    teams = []
        ...
        games.append(
            {
                "href": "/game/" + quote(str(row["event_id"]), safe=":"),
                "opponent": opponent,
                "score": stats_label or "Appearance saved",
                "status": row["team_abbreviation"],
                "start_time": row["start_time"],
            }
        )
    return {
...
def record_sports_ai_forecast(
    ...
    *,
    ...
    try:
        ...
        starts_at = datetime.fromisoformat(str(evidence["start_time"]).replace("Z", "+00:00"))
...
def sports_ai_tournament(league: str = "all") -> dict[str, Any]:
    ...
    current_card = next(
        (card for card in cards if (str(card["actor_id"]), str(card["model"])) == current_identity),
        None,
    )
...
def _sports_alpha_item(
    ...
    *,
    ...
    return {
        ...
        "change_label": (f"{probability_move:+.1f}pp" if probability_move is not None else "—"),
        "change_tone": ("up" if probability_move is not None and probability_move >= 0 else "down"),
...
def sports_alpha_board(league: str = "all", limit: int = 50) -> dict[str, Any]:
    ...
    for pick in picks:
        ...
        group["latest_activity"] = max(str(group["latest_activity"]), str(pick["updated_at"]))
        ...
        calls.append(
            {
                ...
                "return_label": (f"{return_units:+.2f}u" if return_units is not None else None),
                "return_tone": ("up" if return_units is not None and return_units >= 0 else "down"),
