...
def run_verified_pipeline(
    ...
    review_statements = {
        str(finding["statement"]) for review in reviews.values() for finding in review["findings"]
    }
    ...
    verified_unknowns = _clean_strings(
        [unknown for review in reviews.values() for unknown in review["unknowns"]]
    ...
    synthesis_reviews = {
        name: {
            "findings": [
                finding for finding in review["findings"] if finding["statement"] not in rejected
            ],
    ...
    report["catalysts"] = _clean_strings(
        [finding["statement"] for finding in verified_findings if finding["effect"] == "supports"],
        limit=8,
    )
    report["risks"] = _clean_strings(
        [finding["statement"] for finding in verified_findings if finding["effect"] == "risks"],
        limit=8,
    )
    report["watch"] = _clean_strings(
        [finding["statement"] for finding in verified_findings if finding["effect"] == "neutral"]
        + verified_unknowns,
        limit=8,
    )
