...
def saved_cup_history() -> list[dict[str, Any]]:
    with connection() as database:
        rows = database.execute(
            "SELECT snapshot_json,captured_at FROM sports_golf_analysis "
            "WHERE event_id=? ORDER BY captured_at DESC LIMIT 512",
            (EVENT_ID,),
        ).fetchall()
    history = []
    for row in reversed(rows):
        analysis = json.loads(row["snapshot_json"])["analysis"]
        history.append(
            {
