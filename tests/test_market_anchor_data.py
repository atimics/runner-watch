...
def test_sports_filter_counts_show_pending_for_missing_probabilities():
    rows = [
        {
            "id": signal,
    ...
    assert screen["counts"] == {"pending": 4}
    assert all(row["ticker"]["selected"]["percent"] is None for row in screen["rows"])
