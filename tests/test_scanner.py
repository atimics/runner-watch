...
def test_scanner_runs_end_to_end_with_test_frames() -> None:
    now = datetime(2026, 8, 24, 10, 30, tzinfo=EASTERN)
    provider = FakeMarketData(now)
    result = RunnerScanner(provider).scan(
        FAKE_SYMBOLS,
        ScanSettings(min_avg_dollar_volume=0, max_symbols=20, top_n=20),
        now=now,
    )
    assert result.requested_symbols == len(FAKE_SYMBOLS)
    assert result.scanned_symbols == len(FAKE_SYMBOLS)
    assert len(result.rows) == len(FAKE_SYMBOLS)
...
def test_scan_keeps_complete_ranked_candidate_set() -> None:
    ...
    assert len(result.all_rows) == len(FAKE_SYMBOLS)
