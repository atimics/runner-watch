...
def test_sports_forecast_names_the_favorite_and_tag_names_the_value_side(
    ...
    html = BeautifulSoup(response.body, "html.parser")
    assert html.select_one(".sports-forecast b").get_text() == "BOS 63%"
...
def test_selected_team_falls_back_to_full_name(board_context):
    ...
    html = BeautifulSoup(response.body, "html.parser")
    assert html.select_one(".sports-forecast b").get_text() == "Boston Celtics 63%"
    assert html.select_one(".sports-team.is-highlighted").get_text() == "Boston Celtics"
