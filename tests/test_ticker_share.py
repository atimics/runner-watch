...
def test_ticker_share_names_the_symbol_and_the_move() -> None:
    share = share_cards.ticker_share(_ticker_detail())
...
def test_ticker_share_version_tracks_the_latest_quote() -> None:
    first = share_cards.ticker_share(_ticker_detail())
    repriced = share_cards.ticker_share(_ticker_detail(price=13.25))
    requoted = share_cards.ticker_share(_ticker_detail(quote_time="2026-08-24T14:05:00+00:00"))
...
def test_ticker_share_falls_back_when_the_quote_is_missing() -> None:
    share = share_cards.ticker_share(
...
def test_ticker_card_route_and_meta_are_public() -> None:
    ...
    assert "def ticker_share(" in (root / "src/runner_web/share_cards.py").read_text()
...
def test_the_state_badge_keeps_off_the_daily_change_line() -> None:
    ...
    png = share_cards._ticker_card_png(detail)

    image = Image.open(io.BytesIO(png)).convert("RGB")
    assert image.size == (1200, 630)
    date_band = image.crop((640, share_cards.CARD_DATE_Y, 1145, share_cards.CARD_DATE_Y + 24))
    ...
    change_band = image.crop((640, share_cards.CARD_CHANGE_Y, 1145, share_cards.CARD_CHANGE_Y + 30))
...
def test_card_times_are_eastern(stamp, expected):
    assert share_cards._card_time(stamp) == expected
...
def test_the_quote_time_moved_to_the_card_footer_without_the_prefix() -> None:
    ...
    png = share_cards._ticker_card_png(_ticker_detail(quote_time="2026-09-12T15:50:00+00:00"))
    image = Image.open(io.BytesIO(png)).convert("RGB")
    muted = (0x65, 0x71, 0x6B)

    footer = image.crop((700, share_cards.CARD_FOOTER_Y, 1145, share_cards.CARD_FOOTER_Y + 28))
    assert muted in _colors(footer)
    # Nothing is written in the price block any more.
    price_rows = image.crop((700, share_cards.CARD_DATE_Y, 1145, share_cards.CARD_DATE_Y + 24))
...
def test_the_company_name_sits_under_the_ticker_instead_of_over_the_price() -> None:
    ...
    under_ticker = image.crop(
        (95, share_cards.CARD_COMPANY_Y, 700, share_cards.CARD_COMPANY_Y + 30)
    )
    ...
    assert share_cards.CARD_COMPANY_Y + 30 <= share_cards.CARD_DATE_Y
...
def test_the_badge_sits_left_of_the_move_and_inside_the_card() -> None:
    ...
    recorder = Recorder()
    box = share_cards._draw_ticker_badge(
        recorder, detail["current"], right=1000, top=share_cards.CARD_CHANGE_Y
    )
    ...
    assert right == 1000
    assert left < right
    assert top == share_cards.CARD_CHANGE_Y
    assert bottom <= share_cards.CARD_DATE_Y
